...
from pandas import Series, merge, read_csv
...
def main():
    ...
    movie_w_tags_df = (
        merge(left=movies_df, right=tags_df, how="left", on="movieId")
        .groupby(["movieId", "title", "genres"])
        .agg({"tag_user_id": list, "tag_value": list, "tag_timestamp": list})
        .reset_index()
    )

    final_df = (
        merge(left=movie_w_tags_df, right=ratings_df, on="movieId", how="left")
        .groupby(["movieId", "title", "genres"])
        .agg(
            {
                "rating_user_id": list,
                "rating_value": list,
                "rating_timestamp": list,
                "tag_user_id": "first",
                "tag_value": "first",
                "tag_timestamp": "first",
            }
    ...
    final_df["rating"] = final_df[
        ["rating_user_id", "rating_value", "rating_timestamp"]
    ].apply(lambda df: collect_to_dict(df, rating_columns), axis=1)

    tags_columns = ["userId", "value", "timestamp"]

    final_df["tag"] = final_df[["tag_user_id", "tag_value", "tag_timestamp"]].apply(
        lambda df: collect_to_dict(df, tags_columns), axis=1
    )

    records = final_df[["movieId", "title", "genres", "rating", "tag"]].to_dict(
        "records"
    )
