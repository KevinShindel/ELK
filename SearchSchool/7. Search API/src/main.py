    ...
    match args.dsl_query_type:
        ...
        case "query_match":
            request = api.search(
                index=args.index,
                body={
                    "query": {"match": {args.dsl_field: {"query": args.dsl_query_body}}}
                },
            )
            result = request.get("hits", {}).get("hits", [{}])
        ...
        case "fuzzy":
            request = api.search(
                index=args.index,
                body={
                    "query": {"fuzzy": {args.dsl_field: {"value": args.dsl_query_body}}}
                },
            )
            result = request.get("hits", {}).get("hits", [{}])
...
def main(argv=None):
    ...
    parser.add_argument(
        "--index",
        help="Index for search (default: stdin)",
        type=str,
        required=False,
